    ...
    <div className="space-y-6">
      ...
      <div className="p-5 sm:p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">Security ledger listing administrative override actions and data uploads.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="min-h-[30vh] flex items-center justify-center p-8 border border-dashed border-red-500/20 bg-red-500/5 rounded-2xl">
          <div className="text-center max-w-md">
            ...
            <h4 className="font-bold text-base text-red-500 uppercase tracking-widest">Error Loading Audit Logs</h4>
        ...
        <div className="min-h-[30vh] flex items-center justify-center p-8 border border-dashed border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 rounded-2xl">
          <div className="text-center max-w-md">
            <Inbox className="text-slate-400 mx-auto mb-3" size={32} />
            <h4 className="font-bold text-sm text-slate-400 uppercase tracking-widest">No Audit Logs Found</h4>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-950/60 border-b border-slate-200 dark:border-slate-800 text-xs font-bold text-slate-400 uppercase tracking-widest">
                ...
                {logs.map((log) => (
                  <tr key={log._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div>
                        <p className="font-bold text-sm">{log.userId?.name || 'Automated Engine'}</p>
                        <p className="text-xs text-slate-400 capitalize">{log.userId?.role || 'System'}</p>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4 text-right">
                      <span className="text-xs bg-slate-100 dark:bg-slate-800/80 text-slate-500 px-2 py-1 rounded font-mono break-all inline-block max-w-[200px] truncate" title={JSON.stringify(log.details)}>
